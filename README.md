# 🚀 CS-Algorithms-Demystified

Welcome to **Algorithms Demystified**! This repository is built by high school computer science students, *for* high school computer science students. 

Algorithms can seem intimidating, but they are really just step-by-step recipes for solving problems. This repo breaks down the most important foundational algorithms into bite-sized, easy-to-understand pieces.

---

## 📖 What's Inside?

We’ve organized the algorithms by category so you can find exactly what you need. Each folder contains the code, line-by-line comments, and a simple explanation of how it works.

* **🔍 1. Searching:** Finding what you need fast (Linear Search, Binary Search).
* **🗂️ 2. Sorting:** Putting things in order (Bubble, Selection, Insertion, Merge Sort).
* **🔁 3. Recursion & Math:** Functions that call themselves (Fibonacci, Factorials, Euclidean Algorithm).
* **🗺️ 4. Trees, Graphs, & Grids:** Pathfinding and maze solving (Breadth-First Search, Depth-First Search).
* **🔐 5. Strings & Crypto:** Puzzles and encryption (Caesar Cipher, Palindromes).

---

## 🛠️ How to Use This Repository

1.  **Pick a Category:** Browse the folders above to find the algorithm you want to learn.
2.  **Read the Readme:** Each algorithm folder has its own mini-README explaining the concept with a real-world analogy.
3.  **Run the Code:** Open the source code files (e.g., `.java`, `.py`) in your favorite IDE. Play around with it, change the variables, and break things—that's the best way to learn!

---

## 📝 Example Algorithm Page Structure

*(If you are contributing an algorithm to this repo, please follow this format!)*

### Algorithm Name (e.g., Binary Search)
* **The Real-World Analogy:** (e.g., Looking up a name in a phone book by opening it to the middle and tearing it in half).
* **Time Complexity (Big O):** How fast is it? (e.g., `O(log N)`).
* **When to Use It:** The best scenarios for this approach.
* **When NOT to Use It:** The limitations (e.g., The list MUST be sorted first).

---

## 🤝 Contributing

Got a better analogy? Found a bug? Wrote the algorithm in a different programming language? Contributions are always welcome! 

1.  Fork the project.
2.  Create your feature branch (`git checkout -b feature/AmazingAlgorithm`).
3.  Commit your changes (`git commit -m 'Add some AmazingAlgorithm'`).
4.  Push to the branch (`git push origin feature/AmazingAlgorithm`).
5.  Open a Pull Request.

Happy coding! 💻✨
